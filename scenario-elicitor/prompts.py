...
def reflect_after_messages(agent: str, does: str, audience: str, scenario: dict,
    ...
    system = (
        ...
        + _socratic_preamble() +
        f"Write up to {n} questions, of DIFFERENT types, grounded in THEIR OWN "
        "WORDING. Quote or name the specific phrase each question is about. Do "
        "not evaluate the answer and do not suggest a rewrite; a separate rubric "
        "does that. Ask only what makes them examine their own reasoning. Ask "
        "short, plain questions. Use the exact type keys. Do not use em dashes.\n\n"
        f"If what they wrote supports {n} grounded questions, write {n}. If it "
        "is empty, a refusal (like 'skip' or 'idk'), gibberish, or too thin to "
        "ground that many, do not invent positions they never took: ask only "
        "as many questions as their text really supports, even just one, and "
        "make the FIRST question say plainly and kindly that they have not "
        "really written a rule yet, asking what they would want the AI to do "
        "in this situation."
    )
    user = (f"Situation: {scenario.get('situation', '')}\n"
            f"What has to be decided: {scenario.get('at_stake', '')}\n\n"
